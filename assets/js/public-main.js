...
function renderPast(){
  ...
  const list = pastEvents.slice(0, 12);
...
function renderWinners(){
  ...
  if(winnersRail) winnersRail.innerHTML = list.slice(0,18).map(winnerCard).join('');
...
function winnerCard(w){
  ...
  return `<div class="winner" data-win="${w.id}">
...
function attachInteractions(){
  ...
  if(eventSearchInput){
    eventSearchInput.addEventListener('input', debounce(renderUpcoming, 250));
  }
  deptFilterSelect?.addEventListener('change', renderUpcoming);
  if(winnerSearchInput){
    winnerSearchInput.addEventListener('input', debounce(renderWinners, 250));
  }
...
async function onRegisterSubmit(e){
  e.preventDefault();
  const form=e.target;
  const event_id=Number($('#regEventId').value);
