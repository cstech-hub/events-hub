-- SCHEMA + RLS + VIEW + RPC + STORAGE (Idempotent)
...
create policy "auth write event images" on storage.objects for all to authenticated using (bucket_id='event-images') with check (bucket_id='event-images');

create policy "public read winner images" on storage.objects for select to public using (bucket_id='winner');
create policy "auth write winner images" on storage.objects for all to authenticated using (bucket_id='winner') with check (bucket_id='winner');